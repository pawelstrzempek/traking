...
//	double scaleFactor;//this factor scales down all dimentions so they can fit into TCanvas. For
	...
	unsigned hit_qty;
public:
	TStrawModule(double x_coordinate, double y_coordinate, double angle_rotation, unsigned int x, unsigned int y, double straw_radius);
...
TStrawModule::TStrawModule(double x_coordinate, double y_coordinate, double angle_rotation, unsigned int x, unsigned int y, double straw_radius): x_pos(x_coordinate), y_pos(y_coordinate), angle_rot(angle_rotation), raws(x), cols(y), radius(straw_radius) {
...
//	cols > raws ? radius=0.45/cols : radius = 0.45/raws; //we calculated maximal radius which can fit into canvas
	...
	double y_current = y_pos;
        ...
        for (unsigned int x = 0 ; x < cols/2 ; x++){
              for (unsigned int y =0 ; y < raws*2; y++){//we create two raws at once
...
//        for (unsigned int straw_iter = 0; straw_iter< raws*cols; straw_iter++){
			...
			if(angle_rotation == 0){
	         	...
	         	//straw_iter%2 != 0 ? y_pos = y_pos + Constanses::sqrt3*radius : y_pos = y_pos - Constanses::sqrt3*radius;
                        	//straw_iter%2 != 0 ? y_current = y_current + sqrt3*radius : y_current = y_current - sqrt3*radius;
                        	//straw_iter%2 != 0 ? y_current = y_current + sqrt3*radius : y_current = y_current - sqrt3*radius;
				straws.push_back(new TStraw(x_current,y_current,radius));
				//** setting coordinance for the next straw	
				x_current = x_current + radius;
				(y+1)%2 == 0 ? y_current = y_pos+(2*sqrt3*radius)*(x) : y_current = y_pos+(2*sqrt3*radius)*(x) - sqrt3*radius;
			}
			else
			{
	...
	x_current=x_pos;
	y_current=y_pos+(2*sqrt3*radius)*(x+1);
	}
	straws_qty = raws*cols;
